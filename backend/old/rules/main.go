	...
	for scanner.Scan() {
		...
		for _, rule := range rules {
			...
			if matchesRule(log, rule) {
				outMessage := fmt.Sprintf("ALERT: Rule '%s' triggered by log entry at %s\n", rule.RuleName, log.EventTime)
				outMessage += fmt.Sprintf("    SourceImage: %s\n", log.SourceImage)
				outMessage += fmt.Sprintf("    TargetImage: %s\n", log.TargetImage)
				outMessage += fmt.Sprintf("    Message: %s\nEND MESSAGE\n", log.Message)
				outMessage += fmt.Sprintf("    Severity: %s\n\n", rule.Severity)
				_, err := output.WriteString(outMessage)
				if err != nil {
